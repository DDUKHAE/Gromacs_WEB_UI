...
class LLMAdapter(ABC):
    ...
    def build_prompt(self, harness_dir: Path, workspace: Path, pdb_path: Path) -> str:
        return (
            ...
            f"Read {workspace}/resolved_run_plan.json before starting. It contains the user-authoritative settings, selected or auto-detected tutorial, and compatibility status.\n"
            ...
            f"build_environment(\n"
            f"    pdb_path='{pdb_path}',\n"
            f"    prompt='follow the resolved run plan and selected tutorial',\n"
